...
function configPermission(obj){
	...
	$.ajax({
		url:contextPath + "/System/role/getisHavePermission",
		data:{"roleid":roleid},
		type:"POST",
		datatype:"json",
...
function updateroleinfo_submit(){
	$.ajax({
		url:contextPath + "/System/role/updateRoleInfo",
...
function el_configPermission(obj,obj1) {
	...
	var roleid=$(obj).parent("td").find(".hidden_roleid").val();
	$.ajax({
		url:contextPath + "/System/role/getIsUsePermission",
		type:"POST",
		datatype:"json",
		success:function(data){
...
function addRoleinfo(){
	...
	$.ajax({
		url:contextPath + "/System/role/addRoleinfo",
...
function updatePermissioninfo(){
	...
	$.ajax({
		url:contextPath + "/System/role/updatePermissioninfoForRole",
...
function findRoleByDepId(){	
	...
	$.ajax({
		url : contextPath + "/System/role/getRolesByDepartmentId",
...
function getRoleByCondition(){
	...
	$.ajax({
		url : contextPath + "/System/role/getRoleByCondition",
		data:  $("#form_conditionfind_role").serialize(),
		type : 'POST',
		dataType : 'json',
		success : function(data){showRoleInfo(data,i)},
...
function deleteRoles() {
    ...
    if (nums > 0) {
       ...
       $.ajax({
   		url:contextPath + "/System/role/deleteRoleById",
...
function closerole(data) {
    ...
    $.ajax({
		url:contextPath + "/System/role/updateRoleStatus",
...
function showDepartTree() {
    ...
    function onCheck(event,treeId, treeNode) {
        ...
        select_departmentid=[];
        for(var i=0;i<nodes.length;i++){  
        	select_departmentid.push(nodes[i].plantID);  	
        }  
      
        $("#hidden_departmentids").val(select_departmentid);
    	
        className10 = (className10 === "dark" ? "" : "dark");
        el_id = treeNode.plantID;
        
        //判断点击的节点是否被选中，返回false 和 true 
        if (treeNode.checked) {//选中
            showLog10(treeNode.plantName+",",el_id);
        } else {               //点击选中，让其未选中
            noshowLog10(treeNode.plantName+",",el_id);
...
function addMoreRoleWithPermission(){
	...
	$.ajax({
		url:contextPath + "/System/role/addMoreRole",
...
function getPermissionTree(){
	$.ajax({
		url:contextPath + "/System/permission/getPermissionTree",
		type :"POST",
		dataType:"json",
