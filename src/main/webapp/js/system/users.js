...
//alert(contextPath);
//var serverPath = request.getServerName();
...
//	alert(selectedDepartmentID);
	...
	$.ajax({
		url : contextPath + "/System/user/getUserByDepartId",
...
function getUserByCondition(){
	...
	$.ajax({
		url : contextPath + "/System/user/getUserByCondition",
		data:  $("#userinfo_select").serialize(),
		type : 'POST',
		dataType : 'json',
		success : function(data){showUserInfo(data,i)},
	...
	if(userinfo !=null){
		for(var i=0;i<userinfo.length;i++){
			...
			showUserInfo += "<tr><td><input type='checkbox' class='el_checks' name='userids' id='checkbox_userids' value='"+userinfo[i].userid+"'></td><td>"
							...
							+"</td><td class='find_username'>"
					+userinfo[i].username+"</td><td class='find_useridcard'>"
					+userinfo[i].useridcard+"</td><td>"
					+userinfo[i].departmentname+"</td><td class='find_userphone'>"
...
function adduser(){
	...
	$("#adduser_plantId").val(selectedDepartmentID);
//	alert(url);
	$.ajax({
...
function adduser_findempbyname(){
	...
	$.ajax({
		url:contextPath + "/System/user/getEmployeeBynameAnddepartmentid",
...
//	alert(contextPath);
	$.ajax({
		...
		url:contextPath + "/System/user/getDepTreeInfo",
		type :"POST",
		dataType:"json",
...
function beforeClick(treeId, treeNode, clickFlag) {
    ...
    var selectedDepartmentName = treeNode.plantName;
    selectedDepartmentID = treeNode.plantID;
    findUserByDepId();
}
//初始化组合查询的 角色 下拉列表框
function initialRoleSelect(){
	$.ajax({
		url:contextPath + "/System/user/getRolenameAndRoleidByDepartid",
		type :"POST",
		dataType:"json",
		success:function(data){
...
//        $.ajax({
//        	url:contextPath + "/System/user/getEmployeeWithoutUser",
...
function modifyAcc() {
    ...
    if (nums == 1) {
    	...
    	var username = $('input[name=userids]:checked').parents("tr").find(".find_username").text();
    	var userphone = $('input[name=userids]:checked').parents("tr").find(".find_userphone").text();

    	$('#form_update_userinfo_useridcard').val(useridcard);
    	$('#form_update_userinfo_password').val(password);
    	$('#form_update_userinfo_username').val(username);
    	$('#form_update_userinfo_userphone').val(userphone);
...
function modify_password(){
	...
	var url = $.trim($("#url2").val());
	$.ajax({
		url:contextPath + "/System/user/updatePassword",
...
function modify_userinfo(){
	var url = $.trim($("#url2").val());
	$.ajax({
		url:contextPath + "/System/user/updateUserInfo",
		data:$("#form_update_userinfo").serialize(),
		type:"POST",
		datatype:"json",
		success:function(){
...
function el_configRoles(obj,obj1) {
	...
	$.ajax({
		url:contextPath + "/System/user/getIsUseRoleByDepartId",
...
function updateroleinfo(){
	...
	$.ajax({
		url:contextPath + "/System/user/updateRoleinfoForUser",
...
function deleteusers() {
    ...
    if (nums > 1) {
      ...
      $.ajax({
  		url:contextPath + "/System/user/deleteUsersById",
...
function deleteuser(data) {
    ...
    $.ajax({
		url:contextPath + "/System/user/deleteUserById",
		data:{"userid":userid},
		type :"POST",
		dataType:"json",
		success:function(){
