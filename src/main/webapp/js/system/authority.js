...
function initialTreeSelect(){
	//初始化页面的菜单树信息
		...
		$.ajax({
			url:contextPath + "/System/permission/getmenuTreeInfo",
			type :"POST",
			dataType:"json",
...
function findPermissionByMenutop(){	
	...
	$.ajax({
		url : contextPath + "/System/permission/getPermissionBymenutop",
...
function getPermissionByCondition(){
	...
	selectFindMethod='2';
	$.ajax({
		url : contextPath + "/System/permission/getPermissionByCondition",
		data:  $("#form_permissioninfo").serialize(),
		type : 'POST',
		dataType : 'json',
		success : function(data){showPermissionInfo(data,i)},
...
function closepermission(data) {
    ...
    $.ajax({
		url:contextPath + "/System/permission/updatePermissionStatus",
